...
Page({
  onShareAppMessage: function (options) {
    ...
    var url = '/pages/jieqi/pages/jr/jr?content=' + content + '&img=' + img + '&title=' + title + '&date=' + date + '&introduce=' + introduce + '&share=' + share + '&v=' + vip
    console.log(url)
    return {
      title: share,
      desc: '电子贺卡',
  ...
  save:function(e){
    var that = this
    var options = this.data.options
    var title = '['+options.title+']'//传值过来的节日名称
    var date = options.date//节日时间
    var introduce = options.introduce //节日简介
    var content = options.content //节日内容
    ...
    canvasCtx.setFillStyle('#00bab8');
    canvasCtx.setTextAlign('center');
    canvasCtx.fillText("中国节日系列贺卡", 176, 45);
    //绘制右边的点
    canvasCtx.setFontSize(30);
    canvasCtx.setFillStyle('#00bab8');
    canvasCtx.setTextAlign('center');
