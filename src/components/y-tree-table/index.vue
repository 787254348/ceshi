<template>
    <div :style="style">
       ...
       <DxSelection
        ...
        <DxColumn
          ...
          :width="(selection && !sort) ? 80 : 40"
          caption=""
          :fixed="true"
          alignment="left"
          header-cell-template="selection-title"
          :allowResizing="false"
          ...
          :allowFixing="false"
        />
        <template #selection-title="{data}"><div style="cursor: pointer;" @click="openColumn"><em/></div></template>
        ...
        <template v-for="(item,i) in columnData"  #[item.fieldName]="{data}">
          <slot :name="item.fieldName" :data="data">
            <div class="ele-elip" :data-name="item.fieldName">{{data.value}}</div>
          </slot>
        </template>
        <!-- 合计 -->
    <!--    <DxSummary>
    ...
    <span ref="tips" style="cursor: pointer;">
      <a-tooltip>
        ...
        <span class="iconfont icon-guolv" :class="{'margin-left-13':selection}"></span>
      </a-tooltip>
    </span>
</template>
<script setup>
...
import ColumnModal from './column-modal.vue';
  ...
  const props = defineProps({
    ...
    const { style,rowDragging,selection,autoExpandAll,focusedRow,showPage } = toRefs(props);
    ...
    const tips = ref(null);
    setTimeout(()=>{
      let node = grid.value.$el.querySelector('.dx-treelist-headers .dx-treelist-content-fixed .dx-treelist-filter-row td');
    ...
    const renderRecord  = e=>{
      ...
      //设置点击表格时，当前点击元素的横向和纵向的选中效果
       ...
       let dom2 = e.element.querySelectorAll('.dx-fixed-columns .dx-treelist-content-fixed tbody tr');
       //没有固定的表格元素
       dom1.forEach((d,i)=>{
         if(e.rowIndex == i){
           d.style.color = '#fff';
...
<style lang="less" scoped>
  :global(.y-table){
	...
	:deep(.dx-treelist-focus-overlay){
		  border: unset;
	}
  :deep(.dx-treelist-headers .dx-treelist-table .dx-row > td){
    border-bottom: unset;
  }
  :deep(.dx-treelist-content .dx-treelist-table .dx-row > td, .dx-treelist-content .dx-treelist-table .dx-row > tr > td){
  ...
  :deep(.dx-treelist-rowsview.dx-empty .dx-scrollable-content),
  :deep(.dx-treelist-borders > .dx-treelist-filter-panel){
  ...
  :deep(.dx-treelist .dx-treelist-rowsview .dx-freespace-row td.dx-pointer-events-none),
  :deep(.dx-treelist .dx-treelist-headers .dx-row td.dx-pointer-events-none),
  :deep(.dx-treelist-rowsview .dx-data-row td.dx-pointer-events-none){
    border-left: unset;
    border-right: unset;
  }
  :deep(.dx-treelist .dx-treelist-rowsview .dx-freespace-row td.dx-pointer-events-none + td:after),
  :deep(.dx-treelist-rowsview .dx-data-row td.dx-pointer-events-none + td:after),
  :deep(.dx-treelist .dx-treelist-headers .dx-row td.dx-pointer-events-none + td:after){
  ...
  // :deep(.dx-treelist .dx-treelist-rowsview .dx-freespace-row td.dx-pointer-events-none:after),
  // :deep(.dx-treelist-rowsview .dx-data-row td.dx-pointer-events-none:after),
  // :deep(.dx-treelist .dx-treelist-headers .dx-row td.dx-pointer-events-none:after){
  //   box-shadow: inset 10px 0 8px -8px var(--shadow-color);
  //   position: absolute;
  //   top: 0;
  //   bottom: -1px;
  ...
  //   pointer-events: none;
  // }
  // :deep(.dx-treelist .dx-treelist-content-fixed .dx-treelist-table .dx-row td.dx-pointer-events-none){
  //   visibility:unset;
  // }
  // :deep(.dx-treelist .dx-treelist-content-fixed .dx-treelist-table .dx-state-hover td.dx-pointer-events-none){
  //   visibility:hidden;
  // }

  :deep(.dx-treelist-table .dx-data-row.dx-state-hover:not(.dx-selection):not(.dx-row-inserted):not(.dx-row-removed):not(.dx-edit-row):not(.dx-row-focused) > td:not(.dx-row-focused)){
    background-color: #f6faff;
  }
  :deep(.dx-treelist-nodata){
  ...
  :deep(.dx-treelist-rowsview .dx-row-focused.dx-group-row > td){
      background-color: #5c95c5;
      color: #fff;
  }
  :deep(.dx-treelist-filter-panel){
    border-top: unset;
  }
  :deep(.dx-treelist .dx-column-indicators){
    float: right;
  }
  :deep(.dx-column-indicators .dx-header-filter::before){
    content: "" !important;
  }
  :deep(.dx-treelist-headers .dx-treelist-table .dx-row > td:hover){
