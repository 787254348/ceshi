<template>
  <a-modal :width="500" v-model:visible="visible" :centered="true"  title="流程信息" @cancel="cancel" :maskClosable="false" :keyboard="false">
    <div class="margin-bottom-10">流程名称</div>
    <a-input v-model:value="title" placeholder="请输入流程名称" />
    <div class="margin-top-20 margin-bottom-10">流程描述</div>
    <a-textarea v-model:value="describe" placeholder="请输入流程描述" :rows="4" />
    <template #footer>
      <div>
...
<script setup>
  ...
  //取消
  const cancel = ()=> {
    if(title.value.trim() != props.title || describe.value.trim() != props.describe){
      visible.value = true;
      modal.value.open();
    }else{
      close();
