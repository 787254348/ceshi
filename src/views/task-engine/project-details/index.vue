<template>
    <div class="ele-body">
        ...
        <div class="project-centent ant-card">
            <a-tabs v-model:activeKey="activeKey">
              <a-tab-pane v-for="(item,key) in tableData" :key="key" :tab="item.title">
                ...
                >
                  ...
                  <template #is_default="{data}" v-show="activeKey=='task_template' || activeKey=='process'">
                    <a-radio :checked="data.data.is_default" @click.stop="onRadio(data.row.rowIndex)"></a-radio>
                  </template>
                  <template #name="{data}">
                  ...
                  <template #operation="{data}">
                    ...
                    <!-- 流程操作栏 -->
                    <div class="ele-cell justify-center" v-show="activeKey=='process' || activeKey=='rule' || activeKey=='template' || activeKey=='task_template'">
                      ...
                      <div class="margin-left-20 pointer"  @click="handleMenuClick({key:'copy'},data.row.rowIndex)"><span class="iconfont icon-a-zu6112 margin-right-5"></span>复制</div>
           ...
           <!-- 模板配置弹窗 -->
           <taskTemplateDataEdit ref="taskTemplateEditModal" :title="editData.title" :describe="editData.describe" :checked="editData.checked" @save="save"></taskTemplateDataEdit>
           <!-- 消息模板配置弹窗 -->
           ...
           <!-- 工作流设置弹窗 -->
           <workflowSettingsEdit ref="workflowSettingsModel" :title="editData.title" :describe="editData.describe" :checked="editData.checked" @save="save"></workflowSettingsEdit>
        </div>
    </div>
</template>
...
<script setup>
  ...
  import taskTemplateDataEdit from './components/task-template-data-edit.vue';
  import workflowSettingsEdit from './components/workflow-settings-edit.vue';
  const activeKey = ref('task_template');
  //列表数据
  const listData = [{
          id: 1,
  ...
  const columnData = [
    ...
    const columnData1 = [
        ...
        { id: 7, type:'string',fieldName:'update_time', columnName: '修改时间', visible: false,bold:false,width:150,fixed:false,alignment:'center' },
        { id: 8, type:'Boolean',fieldName:'enable', columnName: '启用', visible: true,bold:false,width:100,fixed:false,alignment:'center' },
        { id: 9, type:'Boolean',fieldName:'is_default', columnName: '默认', visible: true,bold:false,width:100,fixed:false,alignment:'center' },
        { id: 10, type:'string',fieldName:'operation', columnName: '设置', visible: true,bold:false,width:300,fixed:false,alignment:'center' },
      ]
    const columnData2 = [
    ...
    const columnData4 = [
        { id: 1, type:'string', fieldName:'project_id', columnName: '模板ID', visible: true,bold:false,width:200,fixed:false,alignment:'center' },
        { id: 2, type:'string', fieldName:'name', columnName: '模板名称', visible: true,bold:false,width:200,fixed:false,alignment:'center' },
        { id: 3, type:'string',fieldName:'project_description', columnName: '模板描述', visible: true,bold:false,width:200,fixed:false,alignment:'center' },
        { id: 4, type:'string',fieldName:'creator', columnName: '创建人', visible: false,bold:false,width:100,fixed:false,alignment:'center' },
        { id: 5, type:'string',fieldName:'creation_time', columnName: '创建时间', visible: false,bold:false,width:150,fixed:false,alignment:'center' },
        { id: 6, type:'string',fieldName:'update_person', columnName: '修改人', visible: false,bold:false,width:100,fixed:false,alignment:'center' },
        { id: 7, type:'string',fieldName:'update_time', columnName: '修改时间', visible: false,bold:false,width:150,fixed:false,alignment:'center' },
  ...
  const taskTemplateEditModal = ref(null);
  const workflowSettingsModel = ref(null);
  ...
  const tableData = reactive({
    task_template:{
      ...
      defalutData:JSON.parse(JSON.stringify(columnData4))
    },
    field:{
      title:'字段',
      listData:listData,
  ...
  const changeEnable = (i)=> {
    ...
    if(activeKey.value=='task_template' || activeKey.value=='process'){
      if(!tableData[activeKey.value].listData[i].enable && tableData[activeKey.value].listData[i].is_default){
        tableData[activeKey.value].listData[i].enable = true;
        message.warning(`默认${tableData[activeKey.value].title}不允许关闭启用`);
      }
    }
  }
  //表格默认单选按钮
  const onRadio = (i)=>{
    if(!tableData[activeKey.value].listData[i].enable){
      message.warning(`请先启用${tableData[activeKey.value].title}`);
    }else{
      tableData[activeKey.value].listData.forEach(item=>{
        item.is_default = false;
      })
      setTimeout(()=>{
        tableData[activeKey.value].listData[i].is_default = true;
      },10)
    }
    console.log(333,i)
  }
  ...
  const add = async ()=> {
    ...
    if(activeKey.value=='process'){
      workflowSettingsModel.value.open();
    }
    ...
    if(activeKey.value=='task_template'){
      taskTemplateEditModal.value.open();
    }
